...
class Product_management extends CI_Controller {
	...
	function add()
	{
		...
		$this->load->view('admin/product_management/add');
	...
	function save()
	{
			...
			$this->form_validation->set_rules('product_name','Product name','trim|required|xss_clean|min_length[1]|max_length[35]');
			if ($this->form_validation->run()==false){
			  ...
			  switch($result){
						...
						$this->load->view('admin/main/header');
						$this->load->view('admin/product_management/add',$aData);
						$this->load->view('admin/main/footer');
						break;
						case 1:
				...
				function view(){
					...
					$this->load->view('admin/main/header');
					$this->load->view('admin/product_management/view',$aData);
					$this->load->view('admin/main/footer');
				}
				...
				function edit($id){
					...
					$this->load->view('admin/main/header');
					$this->load->view('admin/product_management/edit',$aData);
					$this->load->view('admin/main/footer');
				}		
			...
			$this->form_validation->set_rules('product_name','Product name','trim|required|xss_clean|min_length[1]|max_length[35]');
				if ($this->form_validation->run()==false){
				 ...
				 // $aData['row'] =$this->MODEL->edit($id);
				  $result =$this->MODEL->update($this->input->post('product_name'),$id);
